...
**For current draft:**
- `truth-tracking-profiles.tex` (or the largest `*.tex` in the repo root)
...
2. **Skim the main TeX file** (`truth-tracking-profiles.tex` or equivalent) for structure and macro conventions
...
4. **Run a quick build** (optional, if you have TeX installed)
   - `make quick` or `make`
   - otherwise: `latexmk -pdf truth-tracking-profiles.tex`
