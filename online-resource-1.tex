...
\section{Fake-data checks of explanatory surplus}

Constructed data can test whether a proposed model and design recover a known structure and
can reveal cases in which a simpler comparator is sufficient \citep{gelman2020bayesianworkflow}.
They aren't evidence that the decomposition is true. The exercise here instead asks whether
the declared comparison is capable of exposing both success and failure before empirical
...
the route's conditional signal. Let $a$ and $u$ represent
attribution and uptake, $f$ the false-signal probability on an initially correct answer,
$\rho$ the probability that an exercised correction succeeds, and $\eta$ the probability
that an unwarranted intervention causes harm. The structural data-generating model is
\begin{align*}
 ...
 +(1-p_0)P(\text{repair}).
\end{align*}
This multiplication is one deliberately simple data-generating specification, not a
profile score or an assumption that the features are mutually independent. It represents a
serial path whose stages all have to succeed on a correction opportunity.
...
model receives the same true $p_0$; an empirical implementation would estimate that baseline
with uncertainty.
...
an intervention other than the truly best one. Table~\ref{tab:fake-data} reports means over
200 replications from a fixed master seed. Negative differences favour the decomposition;
positive differences favour conditional coverage.

\begin{table}[!t]
\centering
\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}rr>{\raggedright\arraybackslash}p{0.37\linewidth}@{}}
\toprule
Constructed world & $\Delta$ repair loss & $\Delta$ regret & Result \\
\midrule
Uptake bottleneck, accurately measured & $-0.00639$ & $-0.00657$ & The decomposition predicts repair and selects the intervention better. \\
Conditional signal is sufficient & $+0.05695$ & $+0.00348$ & Conditional coverage wins; the proposed attribution and uptake structure is mistaken. \\
True route structure, noisy feature measures & $+0.01478$ & $+0.00409$ & Conditional coverage wins in practice despite the latent structural process. \\
True route structure, no decision surplus & $-0.00415$ & $+0.00001$ & The decomposition predicts repair better but doesn't improve intervention choice. \\
\bottomrule
\end{tabular}
\caption{Paired fake-data comparisons. Differences are decomposition minus conditional
coverage, so lower logarithmic loss and lower regret appear as negative values.}
\label{tab:fake-data}
\end{table}

The first world verifies that the workflow can recover the intended uptake bottleneck. The
other three prevent the exercise from becoming a demonstration by construction. In
particular, better prediction of conditional repair doesn't entail better intervention
choice, and a true but poorly measured decomposition can be less useful than its simpler
comparator. Differences in final-accuracy loss are much smaller because initially correct
answers dominate that aggregate, which is why the article treats repair conditional on error
and harmful intervention as the more diagnostic quantities. The repair and regret
comparisons in each of the first three worlds have the same direction in all 200 replications;
in the fourth, decision regret is effectively tied. The reproducible R script and
machine-readable results accompany this resource.
