...
\caption{Paired fake-data comparisons. Points show individual replications; thin and thick
lines span the central 90 and 50 percent, and circles mark medians. Differences are
decomposition minus conditional coverage. The worlds aren't pooled evidence for one expected
success rate: A is a recovery check, while B--D specify conditions under which the
decomposition can lose.}
\label{fig:fake-data}
\end{figure}

The first world is a recovery check, not evidence of explanatory surplus. Its data-generating
process includes attribution and uptake, while the conditional-coverage comparator
deliberately omits them. The large difference in optimal-choice rate confirms that the
workflow can recover that stipulated bottleneck. Worlds B--D carry the more demanding
burden: the simpler comparator wins when conditional signal is sufficient or the feature
measures are noisy, and better repair prediction needn't improve intervention choice.
World C gives the sharpest practical failure: although route structure generates the data,
noisy feature measures make the decomposition choose the optimal intervention 47.23
percentage points less often.
